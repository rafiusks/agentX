...
impl CommandInput {
    ...
    pub fn get_current_input(&self) -> String {
        // For terminal display, show all content on one line with spaces
        self.buffer.join(" ")
    }
    
    pub fn get_cursor_position(&self) -> usize {
        // Calculate total cursor position across all lines
        let mut pos = 0;
        for (i, line) in self.buffer.iter().enumerate() {
            if i < self.current_line {
                pos += line.len() + 1; // +1 for space/newline separator
